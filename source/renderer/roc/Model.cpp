...
void Model::rotateYaw(float t_delta, bool t_radians) {
  for (size_t i = 0; i < this->mesches.size(); i++) {
    mesches[i]->rotateYaw(t_delta, t_radians);
  }
}

void Model::setYaw(float t_rotation, bool t_radians) {
  for (size_t i = 0; i < this->mesches.size(); i++) {
    mesches[i]->setYaw(t_rotation, t_radians);
  }
}

void Model::setScale(float t_scaleX, float t_scaleY, float t_scaleZ) {
  for (size_t i = 0; i < this->mesches.size(); i++) {
