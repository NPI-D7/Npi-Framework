...
namespace Npi {
Camera::Camera()
    : m_posX(0.0f), m_posY(0.0f), m_posZ(0.0f), m_rotationX(0.0f),
      m_rotationY(0.0f), m_rotationZ(0.0f) { /* do nothing */
}
...
void Camera::setPositionX(float t_position) { m_posX = t_position; }

float Camera::getPositionX() { return m_posX; }

void Camera::setPositionY(float t_position) { m_posY = t_position; }

float Camera::getPositionY() { return m_posY; }

void Camera::setPositionZ(float t_position) { m_posZ = t_position; }

float Camera::getPositionZ() { return m_posZ; }

void Camera::setPosition(float t_positionX, float t_positionY,
                         float t_positionZ) {
  m_posX = t_positionX;
  m_posY = t_positionY;
  m_posZ = t_positionZ;
...
void Camera::setPosition(Npi::Vector3f t_position) {
  m_posX = t_position.x;
  m_posY = t_position.y;
  m_posZ = t_position.z;
}

void Camera::moveX(float t_delta) { m_posX += t_delta; }

void Camera::moveY(float t_delta) { m_posY += t_delta; }

void Camera::moveZ(float t_delta) { m_posZ += t_delta; }
