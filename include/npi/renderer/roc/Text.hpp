...
#include <npi/renderer/Color.hpp>
#include <npi/renderer/ROC.hpp>
#include <npi/renderer/Vertex.hpp>
#include <string>
 ...
 */
class Text : public Npi::ROC {
 public:
  /**
   * @brief Initializes the text
   */
  Text(const std::string& t_text = "",
       Npi::Color t_color = Npi::Color(255, 255, 255));
   ...
   * @note The first component of the vector is the x position, the second
   * component is the y position
   */
  void setPosition(Npi::Vector2f t_vector);
   ...
   * @note The first component of the vector is the offset to move on the
   * x-axis, the second component is the offset on the y-axis
   */
   ...
   */
  Npi::Vector2f getBoundingBox();

  /**
   * @brief Sets the text color
   * @param t_color The color of the text
   */
  void setColor(Npi::Color t_color);
   ...
   * @note If this is enabled, the text won't be drawn from the y-position down
   * but will rather use the y-position as it's baseline
