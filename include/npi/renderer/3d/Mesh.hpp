#pragma once
#include <npi/renderer/3d/Material.hpp>
#include <npi/renderer/Polygon.hpp>
...
class Mesh : public Npi::ROC {
 public:
   ...
   * @param t_resetMaterial Whether or not to reset the material so that it
   * won't interfere with the texture
   */
  void bindTexture(Npi::Texture &t_texture, bool t_resetMaterial = true);

  /**
   * @brief Unbinds the bound texture (disables texture using)
   * @param t_resetMaterial Whether or not to reset the material to the default
   * material
   */
  ...
  inline std::vector<Npi::Polygon::Vertex> GetVertecies() {
 ...
 private:
  /* data */
  float m_rotationX, m_rotationY, m_rotationZ, m_posX, m_posY, m_posZ, m_scaleX,
      m_scaleY, m_scaleZ;
  bool m_useTexture;
