...
Renderer::Renderer(bool _3d)
    : m_3dEnabled(_3d), m_useCulling(true),
      m_clearColorTop(Npi::Color(0, 0, 0)),
      m_clearColorBottom(Npi::Color(0, 0, 0)), m_cameraTop(_1),
      m_cameraBottom(_2), m_useFogTop(false), m_useFogBottom(false),
      m_fogDensityTop(0.05), m_fogDensityBottom(0.05) {
  ...
  u32 flags = GX_TRANSFER_FLIP_VERT(0) | GX_TRANSFER_OUT_TILED(0) |
              GX_TRANSFER_RAW_COPY(0) |
              GX_TRANSFER_IN_FORMAT(GX_TRANSFER_FMT_RGBA8) |
              GX_TRANSFER_OUT_FORMAT(GX_TRANSFER_FMT_RGB8) |
              GX_TRANSFER_SCALING(GX_TRANSFER_SCALE_NO);

  C3D_RenderTargetSetOutput(m_targetTopLeft->getRenderTarget(), GFX_TOP,
                            GFX_LEFT, flags);
  C3D_RenderTargetSetOutput(m_targetTopRight->getRenderTarget(), GFX_TOP,
                            GFX_RIGHT, flags);
  C3D_RenderTargetSetOutput(m_targetBottom->getRenderTarget(), GFX_BOTTOM,
                            GFX_LEFT, flags);
  ...
  // Npi::Shader standart(vshader_shbin, vshader_shbin_size,
  // Npi::Shader::Type::VERTEX);
  rn3d = std::make_unique<Npi::Shader>(rn3d_shbin, rn3d_shbin_size,
                                       Npi::Shader::Type::VERTEX);
  ...
  C3D_LightEnvLut(&m_lightEnvTop, GPU_LUT_D0, GPU_LUTINPUT_LN, false,
                  &m_lutPhongTop);
  ...
  LightLut_Phong(&m_lutPhongBottom, 100);
  C3D_LightEnvLut(&m_lightEnvBottom, GPU_LUT_D0, GPU_LUTINPUT_LN, false,
                  &m_lutPhongBottom);
  lightVec = FVec4_New(0.0f, 0.0f, -0.5f, 1.0f);
...
int Renderer::GetPolyCount() { return polygoncount; }
int Renderer::GetDrawCalls() { return drawcalls; }
...
void Renderer::Render(bool d_clear) {
  ...
  if (m_drawStackTop3d.size() > 0 || m_drawStackBottom3d.size() > 0) {
    ...
    Mtx_PerspStereoTilt(&m_projection, C3D_AngleFromDegrees(40.0f),
                        C3D_AspectRatioBot, 0.01f, 1000.0f, 0, 2.0f, false); 

    C3D_FVUnifMtx4x4(GPU_VERTEX_SHADER, m_projectionUniform, &m_projection);
    C3D_FVUnifMtx4x4(GPU_VERTEX_SHADER, m_viewUniform,
                     &m_cameraBottom.getViewMatrix());
    ...
    if (m_drawStackBottom3d.size() > 0) {
      ...
      if (m_useFogBottom)
        prepareFog(Npi::RenderContext::ScreenTarget::Bottom);
    ...
    if (m_drawStackTop3d.size() > 0) {
      ...
      if (m_useFogTop)
        prepareFog(Npi::RenderContext::ScreenTarget::Top);

      // tilt stereo perspective
      if (m_3dEnabled) {
        Mtx_PerspStereoTilt(&m_projection, C3D_AngleFromDegrees(40.0f),
                            C3D_AspectRatioTop, 0.01f, 1000.0f,
                            -(osGet3DSliderState() / 3.0f), 2.0f, false);
      } else {
        Mtx_PerspStereoTilt(&m_projection, C3D_AngleFromDegrees(40.0f),
                            C3D_AspectRatioTop, 0.01f, 1000.0f, 0, 2.0f, false);
      }

      C3D_FVUnifMtx4x4(GPU_VERTEX_SHADER, m_projectionUniform, &m_projection);
      C3D_FVUnifMtx4x4(GPU_VERTEX_SHADER, m_viewUniform,
                       &m_cameraTop.getViewMatrix());
      ...
      if (m_3dEnabled && osGet3DSliderState() > 0.0f) {
        ...
        if (m_3dEnabled) {
          ...
          Mtx_PerspStereoTilt(&m_projection, C3D_AngleFromDegrees(40.0f),
                              C3D_AspectRatioTop, 0.01f, 1000.0f, 0, 2.0f,
                              false);
        }
...
void Renderer::clear() {
  C3D_RenderTargetClear(m_targetTopLeft->getRenderTarget(), C3D_CLEAR_ALL,
                        __builtin_bswap32(m_clearColorTop.getRgba8()), 0);
  C3D_RenderTargetClear(m_targetTopRight->getRenderTarget(), C3D_CLEAR_ALL,
                        __builtin_bswap32(m_clearColorTop.getRgba8()), 0);
  C3D_RenderTargetClear(m_targetBottom->getRenderTarget(), C3D_CLEAR_ALL,
                        __builtin_bswap32(m_clearColorBottom.getRgba8()), 0);
}
...
void Renderer::useCulling(bool t_useCulling) { m_useCulling = t_useCulling; }

bool Renderer::isUsingCulling() { return m_useCulling; }
...
Npi::Color Renderer::getClearColor(Npi::RenderContext::ScreenTarget t_target) {
  return t_target == Npi::RenderContext::ScreenTarget::Top ? m_clearColorTop
                                                           : m_clearColorBottom;
}

void Renderer::drawTop(Npi::ROC &t_object, Npi::RenderContext::Mode t_mode,
                       int t_layer) {
  if (t_mode == Npi::RenderContext::Mode::_2D) {
    if (m_drawStackTop2d.count(t_layer) > 0) {
      m_drawStackTop2d[t_layer].insert(
          m_drawStackTop2d[t_layer].end(),
          std::make_pair(&t_object, []() { return true; }));
    } else {
      std::vector<std::pair<Npi::ROC *, std::function<bool()>>> newStack;
    ...
    if (m_drawStackTop3d.count(t_layer) > 0) {
      m_drawStackTop3d[t_layer].insert(
          m_drawStackTop3d[t_layer].end(),
          std::make_pair(&t_object, []() { return true; }));
    } else {
      std::vector<std::pair<Npi::ROC *, std::function<bool()>>> newStack;
      newStack.push_back(std::make_pair(&t_object, []() { return true; }));
...
void Renderer::drawTop(Npi::ROC &t_object,
                       std::function<bool()> t_shadingFunction,
                       Npi::RenderContext::Mode t_mode, int t_layer) {
  if (t_mode == Npi::RenderContext::Mode::_2D) {
    if (m_drawStackTop2d.count(t_layer) > 0) {
      m_drawStackTop2d[t_layer].insert(
          m_drawStackTop2d[t_layer].end(),
          std::make_pair(&t_object, t_shadingFunction));
    } else {
      std::vector<std::pair<Npi::ROC *, std::function<bool()>>> newStack;
...
void Renderer::drawBottom(Npi::ROC &t_object, Npi::RenderContext::Mode t_mode,
                          int t_layer) {
  if (t_mode == Npi::RenderContext::Mode::_2D) {
    if (m_drawStackBottom2d.count(t_layer) > 0) {
      m_drawStackBottom2d[t_layer].insert(
          m_drawStackBottom2d[t_layer].end(),
          std::make_pair(&t_object, []() { return true; }));
    } else {
      std::vector<std::pair<Npi::ROC *, std::function<bool()>>> newStack;
      newStack.push_back(std::make_pair(&t_object, []() { return true; }));
    ...
    if (m_drawStackBottom3d.count(t_layer) > 0) {
      m_drawStackBottom3d[t_layer].insert(
          m_drawStackBottom3d[t_layer].end(),
          std::make_pair(&t_object, []() { return true; }));
    } else {
      std::vector<std::pair<Npi::ROC *, std::function<bool()>>> newStack;
      newStack.push_back(std::make_pair(&t_object, []() { return true; }));
...
void Renderer::drawBottom(Npi::ROC &t_object,
                          std::function<bool()> t_shadingFunction,
                          Npi::RenderContext::Mode t_mode, int t_layer) {
  if (t_mode == Npi::RenderContext::Mode::_2D) {
    if (m_drawStackBottom2d.count(t_layer) > 0) {
...
int Renderer::getScreenHeight() { return 240; }

void Renderer::setCamera(Npi::Camera t_camera,
                         Npi::RenderContext::ScreenTarget t_target) {
  switch (t_target) {
...
Npi::Camera &Renderer::getCamera(Npi::RenderContext::ScreenTarget t_target) {
  return (t_target == Npi::RenderContext::ScreenTarget::Top ? m_cameraTop
                                                            : m_cameraBottom);
}

void Renderer::useFog(bool t_useFog,
                      Npi::RenderContext::ScreenTarget t_target) {
...
bool Renderer::getUseFog(Npi::RenderContext::ScreenTarget t_target) {
  return (t_target == Npi::RenderContext::ScreenTarget::Top ? m_useFogTop
                                                            : m_useFogBottom);
}

void Renderer::setFogDensity(float t_density,
                             Npi::RenderContext::ScreenTarget t_target) {
  if (t_target == Npi::RenderContext::ScreenTarget::Top) {
...
float Renderer::getFogDensity(Npi::RenderContext::ScreenTarget t_target) {
  return (t_target == Npi::RenderContext::ScreenTarget::Top
              ? m_fogDensityTop
              : m_fogDensityBottom);
}
...
void Renderer::prepare() {
  ...
  // Configure the first fragment shading substage to blend the fragment primary
  // color with the fragment secondary color. See
  // https://www.opengl.org/sdk/docs/man2/xhtml/glTexEnv.xml for more insight
  C3D_TexEnv *env = C3D_GetTexEnv(0);
  C3D_TexEnvInit(env);
  C3D_TexEnvSrc(env, C3D_Both, GPU_FRAGMENT_PRIMARY_COLOR,
                GPU_FRAGMENT_SECONDARY_COLOR, GPU_PRIMARY_COLOR);
