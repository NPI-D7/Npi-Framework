...
namespace Npi
{
    ...
    void Shader::Load()
    {
        ...
        {
            AttrInfo_AddLoader(&m_attributeInfo, attrinfo[i].regId, attrinfo[i].format, attrinfo[i].count);
        }
    }
    C3D_AttrInfo* Shader::GetAttrInfo()
    {
        return &this->m_attributeInfo;
    }
    void Shader::AddUniformLoader(std::string unfname)
    {
        int smp = shaderInstanceGetUniformLocation(m_shader.vertexShader, unfname.c_str());
    ...
    DVLB_s* Shader::GetDVLB()
    {
        return this->m_dvlb;
    }
    shaderProgram_s* Shader::GetShader()
    {
